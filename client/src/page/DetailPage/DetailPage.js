...
function DetailPage() {
  ...
  const clipRel =
    entry?.clipUrl ?? entry?.clipPath ?? entry?.clip_url ?? entry?.clip ?? null;
  ...
  const posterPath = thumbRel
    ? joinURL(API_BASE, toWebPath(thumbRel))
    : "/image/default.png";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const found = (data.clips || []).find(
          (c) => String(c.clip_id) === String(clipId),
        );
        if (!found) return;
        setEntry({
          id: found.clip_id,
          ...
          message:
            found.class_name === "assault" ? "폭행" : (found.class_name ?? ""),
