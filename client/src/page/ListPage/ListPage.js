...
function ListPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const latestRes = await fetch(
        `${API_BASE}/jobs/latest?username=${encodeURIComponent(username)}`,
      );
      if (!latestRes.ok) {
  ...
  const handleClick = async (entry) => {
    try {
      ...
      // 2) 프론트 상태 동기화
      setEntries((prev) =>
        prev.map((e) => (e.id === entry.id ? { ...e, checked: true } : e)),
      );
                ...
                <SuspectEntry key={entry.id} onClick={() => handleClick(entry)}>
                  ...
                  <SuspectDetail>
                    <IncidentInfo>
                      ...
                      <DetailMessage>
                        <span style={{ color: "red" }}>{entry.message}</span>
                        이(가) 발생했습니다.
                      </DetailMessage>
                    </IncidentInfo>
