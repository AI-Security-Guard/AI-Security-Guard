    ...
    def __init__(self, args):
        ...
        if self.motion:
            ...
            self.correlation_sampler = SpatialCorrelationSampler(
                kernel_size=1, patch_size=self.ps, stride=1, padding=0, dilation_patch=1
            )

            self.corr_output_proj = nn.Sequential(
                nn.Conv2d(
                    self.ps * self.ps, self.channel_dim, kernel_size=1, bias=False
                ),
                nn.ReLU(),
            )
...
class MultiCorrBackbone(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.corr_output_proj1 = nn.Sequential(
            nn.Conv2d(
                self.ps * self.ps, self.layer_channel[2], kernel_size=1, bias=False
            ),
            nn.ReLU(),
        )
        self.corr_output_proj2 = nn.Sequential(
            nn.Conv2d(
                self.ps * self.ps, self.layer_channel[3], kernel_size=1, bias=False
            ),
            nn.ReLU(),
        )
...
def build_backbone(args):
    ...
    # ⚠️ 여기서 직접 resnet 인스턴스를 만들어야 함
    backbone_instance = backbone_model(
        replace_stride_with_dilation=[False, False, args.dilation], weights=weights
    )

    # motion 있는 경우 backbone instance는 내부에서 다시 씀
    if args.multi_corr:
        backbone = MultiCorrBackbone(args)
    else:
        # 여기서 custom backbone에 위에서 만든 인스턴스를 넣는 방식은 아님.
